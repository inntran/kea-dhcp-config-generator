...
def _cfg(dhcp6_dict: dict) -> GlobalConfig:
    return GlobalConfig.model_validate({"dhcp6": dhcp6_dict})
...
def test_na_pool_auto_range():
    ...
    assert subnet["pools"] == [
        {"pool": "2001:db8:1::1 - 2001:db8:1:0:ffff:ffff:ffff:fffe"}
    ]
...
def test_pd_pool_format():
    ...
    subnet = build(config)["Dhcp6"]["subnet6"][0]
    assert subnet["pd-pools"] == [
        {"prefix": "2001:db8:1::", "prefix-len": 48, "delegated-len": 64}
    ]
...
def test_mixed_na_and_pd_pools_split_into_two_keys():
    config = _cfg({
        "subnets": [{
            ...
            "pools": [
                _na_pool("auto"),
                {"pool-type": "pd", "prefix": "2001:db8:1::",
